...
//Alineamiento para que todo ande correcto
...
AutoPlay.run = function() {
  ...
  if (AutoPlay.plantPending)
    AutoPlay.addActivity("Asegurate de cosechar la nueva planta antes de ascender!");
  ...
  if (!Game.HasAchiev('Elder')) AutoPlay.addActivity("Obteniendo los 7 tipos de Abuelas");
  if (Game.HasAchiev('Elder') && Game.Upgrades['Bingo center/Research facility'].unlocked &&
    ...
    AutoPlay.addActivity("Avanzando con el complejo de investigación de las Abuelas");
}
...
AutoPlay.runJustRight = function() {
  ...
  AutoPlay.activities = "Todo va en orden..";
  AutoPlay.handleAscend();
...
//  if (AutoPlay.preNightMode()) AutoPlay.setDeadline(0); // not needed
  ...
  if (AutoPlay.night) { //really sleep now
    AutoPlay.activities = 'Liorecito está durmiendo..';
    return true;
  }
  AutoPlay.addActivity('Preparandose para la noche..');
  AutoPlay.nightAtGarden(true);
  AutoPlay.nightAtStocks();
  var gs = Game.Upgrades["Golden switch [off]"];
  if (gs.unlocked) {
    AutoPlay.handleGoldenCookies();
    AutoPlay.addActivity('Esperando al mejor momento para comprar Golden switch.');
...
AutoPlay.cheatGoldenCookies = function() {
  ...
  AutoPlay.addActivity('Chitiando galletas doradas al nivel ' + level + '.');
...
AutoPlay.handleSavings = function() {
  ...
  if (elapsedTime < 0) {
    AutoPlay.savingsGoal = 0;
    AutoPlay.addActivity('Sin ahorros por los primeros ' + (startTime / 60 / 1000) +
          '+ minutos!');
    return;
  }
...
AutoPlay.bestBuy = function() {
  ...
  if (!haveBought) {
    ...
    AutoPlay.addActivity('Esperando a comprar ' + best);
  }
}
...
AutoPlay.handleSeasons = function() {
  ...
  if (AutoPlay.seasonFinished(Game.season)) {
    ...
    AutoPlay.addActivity('Esperando para los resultados en '+Game.season+'.');
}
...
AutoPlay.cheatSugarLumps = function(age) {
  ...
  AutoPlay.addActivity('Chitiando terrones de azúcar..');
...
AutoPlay.gardenSacrificeReady = function(g) {
  ...
  if (!Game.AchievementsById[382].won && g.plantsUnlockedN==g.plantsN) {
    ...
    AutoPlay.addActivity('Esperando para cosechar antes de quedarse sin semillas que plantar..');
  }
  return false;
}
...
AutoPlay.planting = function(game) {
  if (!game.plants["meddleweed"].unlocked) {  // wait for meddleweed
    AutoPlay.plantList=[0,0,0,0];
    AutoPlay.addActivity("Esperando para las meddleweed.");
    AutoPlay.switchSoil(game,0,'fertilizer');
    return;
  }
  if (!game.plants["crumbspore"].unlocked || !game.plants["brownMold"].unlocked) {
    AutoPlay.addActivity("Intentando obtener crumbspore y brown mold."); // use meddleweed
    for (var x = 0; x<6; x++) for (var y = 0; y<6; y++)
...
AutoPlay.plantSeeds = function(game, targets) {
  ...
  if (AutoPlay.cpsMult > 1+10*(AutoPlay.grindingCheat()+(AutoPlay.Config.CheatGolden>1))) {
    AutoPlay.addActivity("No compres plantas ahora - son muy caras.");
    return; // do not plant when it is expensive
  }
  // calculate costs
  let cost = 0;
...
AutoPlay.seedCalendar = function(game,sector) {
  ...
  if (!Game.Upgrades["Green yeast digestives"].unlocked &&
    ...
    AutoPlay.switchSoil(game,sector,'fertilizer');
    if (doPrint) AutoPlay.addActivity("Intentando obtener Green yeast digestives..");
    AutoPlay.plantCookies = true;
    return "greenRot";
  }
  if (!Game.Upgrades["Duketater cookies"].unlocked &&
    ...
    AutoPlay.switchSoil(game,sector,'fertilizer');
    if (doPrint) AutoPlay.addActivity("Intentando obtener Duketater cookies..");
    AutoPlay.plantCookies = true;
    return "duketater";
  }
  if (!Game.Upgrades["Elderwort biscuits"].unlocked &&
    ...
    AutoPlay.switchSoil(game,sector,'fertilizer');
    if (doPrint) AutoPlay.addActivity("Intentando obtener Elderwort cookies..");
    AutoPlay.plantCookies = true;
    return "elderwort";
  }
  if (!Game.Upgrades["Bakeberry cookies"].unlocked &&
    ...
    AutoPlay.switchSoil(game,sector,'fertilizer');
    if (doPrint) AutoPlay.addActivity("Intentando obtener Bakeberry cookies..");
    AutoPlay.plantCookies = true;
    return "bakeberry";
  }
  if (!Game.Upgrades["Wheat slims"].unlocked &&
    ...
    AutoPlay.switchSoil(game,sector,'fertilizer');
    if (doPrint) AutoPlay.addActivity("Intentando obtener Wheat Slims..");
    AutoPlay.plantCookies = true;
    return "bakerWheat";
  }
  if (!Game.Upgrades["Fern tea"].unlocked &&
    ...
    AutoPlay.switchSoil(game,sector,'fertilizer');
    if (doPrint) AutoPlay.addActivity("Intentando obtener Fern Tea..");
    AutoPlay.plantCookies = true;
    return "drowsyfern";
  }
  AutoPlay.plantCookies = false;
    ...
    if (game.isTileUnlocked(x,y)) {
      ...
      if (tile[0]) {
        ...
        if (!plant.unlocked) {
          ...
          AutoPlay.addActivity(plant.name + " aún está creciendo, no interrumpir!");
...
AutoPlay.handleWrinklers = function() {
  ...
  if (doPop) {
    ...
    AutoPlay.addActivity("Explotando sanguijuelas por recompensas y/o logros..");
    Game.wrinklers.forEach(function(w) { if (w.close==1) w.hp = 0; } );
  } else {
    AutoPlay.findNextWrinkler();
    AutoPlay.addActivity("Explotando una sanguijuela cada 2 horas, última hace " +
      (((AutoPlay.now-AutoPlay.wrinklerTime)/1000/60)>>0) + " minutos.");
...
AutoPlay.handleSmallAchievements = function() {
  ...
  if (!Game.Achievements["God complex"].won) {
    Game.bakeryName = "Liorecito"; Game.bakeryNamePrompt(); Game.ConfirmPrompt();
...
AutoPlay.handleAscend = function() {
  ...
  if (Game.ascensionMode==1 && !AutoPlay.canContinue() && !Game.AchievementsById[AutoPlay.nextAchievement].won) {
    AutoPlay.doAscend("Modo renacimiento no funcionó, reintentado..",0);
    return;
  }
  ...
  if (AutoPlay.endPhase() && !Game.Achievements["Endless cycle"].won &&
    ...
    AutoPlay.activities = "Yendo por 1000 ascensiones.";
    ...
    if ((Game.ascendMeterLevel>0) /*&&
      ...
      AutoPlay.doAscend("Yendo por 1000 ascensiones",0);
      return;
    }
  }
  if (Game.Upgrades["Permanent upgrade slot V"].bought &&
    ...
    AutoPlay.activities = "Yendo por 100 ascensiones.";
    AutoPlay.hyperActive=true; // full activity
    AutoPlay.wantAscend = true; //avoid buying plants
    if (Game.ascendMeterLevel>0 &&
        AutoPlay.ascendLimit<Game.ascendMeterLevel*Game.ascendMeterPercent) {
      AutoPlay.doAscend("Yendo por 100 ascensiones",0);
      return;
    }
  }
  var daysInRun = (AutoPlay.now-Game.startDate)/1000/60/60/24;
  if (AutoPlay.nextAchievement==463 && daysInRun > 10 && Game.Objects["Bank"].minigame.profit > daysInRun*300000) {
    AutoPlay.addActivity("Haciendo dinero en el mercado de reservas para obtener logros..");
  } else {
    var maxDaysInRun =
          40*(Game.prestige+1000000000)/(Game.prestige+Game.ascendMeterLevel);
    if (!AutoPlay.wantAscend && daysInRun>20)
      AutoPlay.addActivity("Aún faltan " + ((maxDaysInRun-daysInRun)<<0) +
          " días antes de la siguiente ascensión forzada..");
    if (daysInRun>maxDaysInRun && daysInRun>20) {
      ...
      if (x<9) {
        AutoPlay.doAscend("ascendiendo luego de " + (daysInRun<<0) +
            " días mientras esperamos a obtener el siguiente logro..",1);
        return;
      }
    }
  }
  ...
  if (!Game.Upgrades["Lucky payout"].bought && Game.heavenlyChips>77777777) {
    ...
    // cannot get just one heavenly chip
      if (!AutoPlay.lastPrestige) AutoPlay.info("Es imposible obtener lucky payout - chiteando..");
      AutoPlay.lastPrestige=Game.prestige%1000000;
    }
    AutoPlay.wantAscend = true; //avoid buying plants
    AutoPlay.hyperActive=true; //full activity
    AutoPlay.addActivity("Intentando obtener mejora celestial: Lucky Payout.");
    if (Game.ascendMeterLevel>0 && Game.prestige%1000000 < 777777 &&
        (newPrestige+Game.ascendMeterLevel >= 777777)) {
      AutoPlay.doAscend("intentando obtener la mejora celestial lucky payout.",0);
      return;
    }
...
AutoPlay.canContinue = function() {
  ...
  if (!Game.Achievements["True Neverclick"].won && Game.cookieClicks==0) {
    AutoPlay.addActivity("Intentando obtener el logro: True Neverclick.");
    if (AutoPlay.neverclickWarn)
	  Game.Prompt('<h3>Attention</h3><div class="block">'+
      '<p>Liorecito está intentando obtener el logro True Neverclick</p>'+
      '<p>No clickees galletitas</p>'+
      '</div>',['OK']);
    AutoPlay.neverclickWarn=false;
    needAchievement = true;
  }
  if (!Game.Achievements["Neverclick"].won && Game.cookieClicks<=15) {
    AutoPlay.addActivity("Intentando obtener el logro: Neverclick.");
    needAchievement = true;
  }
  if (!Game.Achievements["Hardcore"].won && Game.UpgradesOwned==0) {
    AutoPlay.addActivity("Intentando obtener el logro: Hardcore.");
    needAchievement = true;
  }
  ...
  if (!Game.Achievements["Speed baking I"].won &&
    ...
    AutoPlay.addActivity("Intentando obtener el logro: Speed baking I.");
    AutoPlay.addActivity("Intentando obtener el logro: Speed baking II.");
    AutoPlay.addActivity("Intentando obtener el logro: Speed baking III.");
...
AutoPlay.doAscend = function(str,log) {
  ...
  AutoPlay.addActivity("Preparamdp ascensión");
...
AutoPlay.grinding = function() {
  ...
  if (Game.AchievementsById[grindingStart].won) { // grind for the last 7 big achievements
    if (!AutoPlay.endPhase()) {
      AutoPlay.addActivity('Farmeando galletitas, no se duerme pa');
      return true;
    }
  }
  return false;
...
AutoPlay.mainActivity = "Descansando..";
AutoPlay.activities = AutoPlay.mainActivity;
...
AutoPlay.findNextAchievement = function() {
  ...
  for (var i = 0; i<AutoPlay.wantedAchievements.length; i++) {
    if (!(Game.AchievementsById[AutoPlay.wantedAchievements[i]].won)) {
      ...
      AutoPlay.setMainActivity("Intentando obtener los siguientes logros: " +
        Game.AchievementsById[AutoPlay.nextAchievement].desc.replace(/<q>.*?<\/q>/ig, ''));
      return;
    }
  }
...
AutoPlay.checkAllAchievementsOK = function() { //We do not stop for one-year legacy
  for (var i in Game.Achievements) {
    ...
    if (!me.won && me.pool!="dungeon" && me.id!=367 && !AutoPlay.lateAchievements.includes(me.id)) { // missing achievement
      AutoPlay.setMainActivity("Falta el logro #" + me.id +
        ": " + me.desc.replace(/<q>.*?<\/q>/ig, '') + ", intentando obtenerlo..");
  ...
  for (var i in Game.Upgrades) {
    ...
    if (me.pool=='prestige' && !me.bought) { // we have not all prestige upgrades yet
      ...
      AutoPlay.setMainActivity("Mejorando Prestigio " + me.name +
        " faltante, esperando para comprar..");
//      Game.RemoveAchiev(Game.AchievementsById[AutoPlay.nextAchievement].name);
  ...
  AutoPlay.setMainActivity("Terminé mi trabajo por hoy, que tengas un buen día, iré a descansar..");
  AutoPlay.nextAchievement = 99; // follow the white rabbit (from dungeons)
  return false;
}
  ...
  if (Game.dragonLevel>=8) { // can pet the dragon
    ...
    for (var drop of drops) {
      if (!Game.Has(drop) && !Game.HasUnlocked(drop)) { // still something we can get
        AutoPlay.addActivity("Domando al dragón...");
        Game.specialTab = "dragon";
...
AutoPlay.ConfigData.BotMode =
  {label: ['Dormido', 'Automático', 'Manual'], desc: 'Configuración general de Liorecito'};
AutoPlay.ConfigData.NightMode =
  {label: ['Desactivado', 'Automático', 'Activado'], desc: 'Manejo de Liorecito, sí duerme, se encarga el mismo de decidir o está durmiendo'};
AutoPlay.ConfigData.ClickMode =
  {label: ['Desactivados', 'Normales', 'Rápidos', 'Muy rápidos', '???'],
   desc: 'Velocidad de Clics'};
AutoPlay.ConfigData.GoldenClickMode =
  {label: ['Activado', 'Automático', 'Recoger todas'], desc: 'Recoger galletas doradas'};
AutoPlay.ConfigData.SavingStrategy =
  {label: ['Sin Ahorrar', 'Automático', 'Ahorro moderado', 'Gastar casi todo'],
   desc: 'Estrategia de ahorros'};
AutoPlay.ConfigData.CheatLumps =
  {label: ['Desactivado', 'Automático', 'Poquitos', 'Moderadamente', 'Demasiados'], desc: 'Chitiar terrones de azúcar'};
AutoPlay.ConfigData.CheatGolden =
  {label: ['Desactivado', 'Automático', 'Poquitas', 'Moderadamente', 'Demasiadas'], desc: 'Chitiar galletas doradas'};
AutoPlay.ConfigData.CleanLog = {label: ['Limpiar registro'], desc: ' ' };
AutoPlay.ConfigData.ShowLog = {label: ['Mostrar registro'], desc: ' '};
...
AutoPlay.setBotMode = function() {
  ...
  AutoPlay.info("Liorecito cambió al modo "+AutoPlay.ConfigData.BotMode.label[AutoPlay.Config.BotMode]);
...
AutoPlay.status = function(print=true) { // just for testing purposes
  ...
  for (var a in Game.Achievements) {
    ...
    if (!me.won && me.pool!="dungeon") {
      if (print) AutoPlay.info("Falta el logro #" + me.id +
  ...
  for (var i in Game.Upgrades) {
    ...
    if (!me.bought && me.pool!="debug" && me.pool!="toggle") {
      ...
      if (print) AutoPlay.info("Mejora " + me.name + " faltante.");
      up++;
    }
  }
  for (var o in Game.Objects) {
    ...
    if (print && myl) AutoPlay.info(""+myl+" faltan terrones de azúcar para " + me.name + ".");
	lum+=myl;
  }
  lum-=Game.lumps;
  if (lum<0) lum=0;
  AutoPlay.addActivity("Faltan "+(ach)+" logros ("+sach+" sombra), "+up+" mejoras, y "+lum+" terrones de azúcar.");
}
...
AutoPlay.logging = function() {
  ...
  try {
    ...
    var toAdd = "Liorecito está guardando los progesos en la versión " + AutoPlay.version + " con " +
                AutoPlay.loggingInfo + "\n" + Game.WriteSave(1) + "\n";
    AutoPlay.loggingInfo = 0;
    window.localStorage.setItem("Registro de actividad",before+toAdd);
  } catch (e) {}
}

AutoPlay.cleanLog = function() {
  try {
    window.localStorage.setItem("Registro de actividad","");
  } catch (e) {}
}
...
AutoPlay.showLog = function() {
  ...
  var str=
    Game.Prompt('<h3>Registro de actividades</h3><div class="block">'+
      'Este es el registro de Liorecito con guardados en etapas importantes<br>'+
      'Copiar a gusto.</div>'+
...
AutoPlay.whatTheBotIsDoing = function() {
  return '<div style="padding:8px;width:400px;font-size:11px;text-align:center;">'+
    '<span style="color:#6f6;font-size:18px"> Que estará haciendo Liorecito?</span>'+
    '<div class="line"></div>'+
    AutoPlay.activities+
    '</div>';
}
...
if (AutoPlay.autoPlayer) {
  AutoPlay.info("Reemplazando versión");
